...
def f_default_arg_mutable(x: int, data: list = None) -> list:
    if data is None:
        data = []
    if len(data) < 3:
        data.append(x)
...
def f_str_formatting(array: List[float]) -> str:
    a = "{:.2f}".format(array[0])
    for v in array[1:]:
        a = a + " - {:.2f}".format(v)
    return a
...
def f_logic(x: float, b1: bool, b2: bool) -> bool:
    return (1 < x < 4) or (b1 is True and b2 is False)
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    return sum(x for x in range(2, 11) if x != 5 and x != 7), set(x for x in dct.keys() if 1 <= dct[x] <= 5)
