...
# DONEFIXME: Create a linear SVC classifier (with C = 0.1).
clf = svm.SVC(kernel='linear', C=0.1, random_state=1, probability=True)

# DONEFIXME: Fit the SVC model using the training data.
clf.fit(X_train, y_train)

# DONEFIXME: Predict the labelling.
y_pred_train = clf.predict(X_train)

# DONEFIXME: Compute the training accuracy.
good_pred = 0
all = 0
for true, pred in zip(y_train, y_pred_train):
    ...
    if true == pred:
        good_pred += 1
acc_train = good_pred/all
print('Training Accuracy: {:.2f}%\n'.format(acc_train * 100))

# %% =================== Part 4: Test Spam Classification ================
#  After training the classifier, we can evaluate it on a test set.

# DONEFIXME: Load the test dataset ('data/spamTest_X.csv', 'data/spamTest_y.csv').
...
# DONEFIXME: Print the list of 15 most prominent features.
...
for i in range(15):
    # DONEFIXME: Replace each `None` with an appropriate expression.
    print(' {word:<20}: {weight:10.6f}'.format(
        word=vocabulary_dict[idx[i] + 1], weight=weights[idx[i]]))
